...
var IntifaceProcessManager = class {
	...
	async start(signal) {
		...
		try {
			...
			if ((typeof error === "object" && error !== null ? error.code : void 0) !== "ENOENT" || !this.config.autoDownload) throw new ToyError(`Could not start Intiface Engine (${this.executable}): ${error instanceof Error ? error.message : String(error)}`);
...
//#endregion
export { AutoToyBackend, ButtplugBackend, Config, IntifaceProcessManager, ManagedButtplugBackend, MonsterPartyBackend, ToyError, ToyRuntime, apply, extractIntifaceExecutable, inject, installIntifaceEngine, name, parseButtplugDeviceList, resolveConfig, routeToyTarget, selectIntifaceArtifact };
