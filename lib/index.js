	...
	async start(signal) {
		...
		try {
			await new Promise((resolve, reject) => {
				...
				const onError = (error) => {
					finish(new ToyError(`Could not start Intiface Engine (${this.config.executable}): ${error.message}`));
				};
				const onAbort = () => {
					child.kill();
					finish(signal.reason instanceof Error ? signal.reason : new DOMException("Aborted", "AbortError"));
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		name: "toy_connect",
		description: "Before the first connection, ask the user for the exact toy brand and model. Never ask the user to choose a connection backend or to start Intiface. Pass the answer here: the tool selects the connection method and starts local Intiface Engine automatically when needed. Secrets come only from plugin config.",
		parameters: {
			model: {
				type: "string",
				required: true,
				description: "Exact product model reported by the user; do not guess it."
			},
			brand: {
				type: "string",
...
//#endregion
export { AutoToyBackend, ButtplugBackend, Config, IntifaceProcessManager, ManagedButtplugBackend, MonsterPartyBackend, ToyError, ToyRuntime, apply, inject, name, parseButtplugDeviceList, resolveConfig, routeToyTarget };
